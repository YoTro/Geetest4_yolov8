...
`libs/tools/end2end`目录下存放了文本检测+文本识别pipeline串联预测的指标评测代码以及可视化工具。本节介绍文本检测+文本识别的端对端指标评估方式。
...
运行`libs/tools/infer/predict_system.py`，得到保存的结果:

```
python3 libs/tools/infer/predict_system.py  --det_model_dir=./ch_PP-OCRv2_det_infer/ --rec_model_dir=./ch_PP-OCRv2_rec_infer/  --image_dir=./datasets/img_dir/ --draw_img_save_dir=./ch_PP-OCRv2_results/ --is_visualize=True
```
...
修改 `libs/tools/end2end/convert_ppocr_label.py`中的代码，convert_label函数中设置输入标签路径，Mode，保存标签路径等，对预测数据的GTlabel和预测结果的label格式进行转换。

```
python3 libs/tools/end2end/convert_ppocr_label.py --mode=gt --label_path=path/to/label_txt --save_folder=save_gt_label

python3 libs/tools/end2end/convert_ppocr_label.py --mode=pred --label_path=path/to/pred_txt --save_folder=save_PPOCRV2_infer
```
...
执行端对端评测，运行`libs/tools/eval_end2end.py`计算端对端指标，运行方式如下：

```
python3 libs/tools/eval_end2end.py "gt_label_dir"  "predict_label_dir"
```

比如：

```
python3 libs/tools/eval_end2end.py ./save_gt_label/ ./save_PPOCRV2_infer/
```
