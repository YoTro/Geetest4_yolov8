...
class ImageMatcher:
    ...
    def _load_dictionary(self, use_cache):
        ...
        if not os.path.exists(self.dictionary_path):
            print(f"警告：字典目录 '{self.dictionary_path}' 不存在。ImageMatcher 将无法工作。")
            print("请先运行 'training/generate_char_dictionary.py' 来生成字典。")
            return dictionary_data

        for filename in os.listdir(self.dictionary_path):
            if filename.startswith('char_') and filename.endswith('.png'):
                try:
                    char = filename.split('_')[1].split('.')[0]
                    ...
                    print(f"加载或处理字典图片 '{filename}' 失败: {e}")
        return dictionary_data
    ...
    def find_best_match(self, target_image_path, weights=None):
        ...
        # --- 第一阶段: 使用 proj 进行快速粗筛 ---
        TOP_N_CANDIDATES = 128  # 定义粗筛后保留的候选项数量
